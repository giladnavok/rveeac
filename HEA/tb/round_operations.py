...
def mix_single_column(col: list[int]) -> list[int]:
    """
    Perform the AES MixColumns on a single 4-byte column.
    Uses:
    ...
    return [
        b0 ^ c1 ^ a2 ^ a3,  # 2·a0 ⊕ 3·a1 ⊕ 1·a2 ⊕ 1·a3
        a0 ^ b1 ^ c2 ^ a3,  # 1·a0 ⊕ 2·a1 ⊕ 3·a2 ⊕ 1·a3
        a0 ^ a1 ^ b2 ^ c3,  # 1·a0 ⊕ 1·a1 ⊕ 2·a2 ⊕ 3·a3
        c0 ^ a1 ^ a2 ^ b3   # 3·a0 ⊕ 1·a1 ⊕ 1·a2 ⊕ 2·a3
    ]
    
def sub_bytes(state: list[list[int]]) -> None:
    """In-place SubBytes on a 4×4 state matrix."""
    for r in range(4):
        for c in range(4):
            state[r][c] = SBOX[state[r][c]]

def shift_rows(state: list[list[int]]) -> None:
    """In-place ShiftRows on a 4×4 state matrix."""
    for r in range(4):
        state[r] = state[r][r:] + state[r][:r]

def mix_columns(state: list[list[int]]) -> None:
    """In-place MixColumns on a 4×4 state matrix."""
    # operate on columns
    for c in range(4):
        # extract column
...
def bytes_to_state(block: bytes) -> list[list[int]]:
    """Convert 16-byte block into 4×4 state matrix (row, col)."""
    assert len(block) == 16
    # AES uses column-major: block[0]→state[0][0], block[1]→state[1][0], …
    return [[block[r + 4*c] for c in range(4)] for r in range(4)]

def state_to_bytes(state: list[list[int]]) -> bytes:
    """Flatten 4×4 state matrix back to 16-byte block."""
...
if __name__ == "__main__":
    ...
    print(state_to_bytes(state).hex())
    
    shift_rows(state)
    print("After SubBytes→ShiftRows:")
    print(state_to_bytes(state).hex())
    
    mix_columns(state)
    print("After SubBytes→ShiftRows→MixColumns:")
    print(state_to_bytes(state).hex())
